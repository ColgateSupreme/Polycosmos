...
mirror_upgrade_items = [    
    ...
    MirrorUpgradeData("Thick Skin", 10),
    MirrorUpgradeData("Privileged Status", 2),
    MirrorUpgradeData("Olympian Favor", 10), # We will make each item give 4 levels, so we only need 10 items to max this out
    MirrorUpgradeData("Gods' Pride", 10), # Likewise, this will be 2 levels per item, so we need 10
    MirrorUpgradeData("Fated Authority", 8),
    MirrorUpgradeData("Fiery Presence", 5),
    MirrorUpgradeData("Dark Regeneration", 2),
    MirrorUpgradeData("Stubborn Defiance", 1),
